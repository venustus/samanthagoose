...
class TextExtractions {
  ...
  @Test
  def positivity(): Unit = {
    val url = "http://www.powerofpositivity.com/5-life-changing-things-that-happen-when-you-downsize/"
    implicit val config = TestUtils.NO_IMAGE_CONFIG
    val article = TestUtils.getArticle(url, getHtml("positivity.txt"))
    assertTrue("There should be 15 paragraphs in this article", article.paragraphs.size == 15)
