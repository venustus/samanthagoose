...
import com.gravity.goose.outputformatters.{ParagraphOutputFormatter, StandardOutputFormatter, OutputFormatter}
...
class Crawler(config: Configuration) {
  ...
  def crawl(crawlCandidate: CrawlCandidate): Article = {
    ...
    for {
      ...
      val paragraphOutputFormatter = ParagraphOutputFormatter
      ...
      extractor.calculateBestNodeBasedOnClustering(article) match {
        case Some(node: Element) => {
          ...
          article.paragraphs = paragraphOutputFormatter getParagraphs (article.topNode)
